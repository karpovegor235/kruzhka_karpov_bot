...
async function sendMorningPractice() {
    try {
        ...
        const userName = getUsersName(users[0]);
...
function getUsersName(userId) {
    ...
    return user ? user.first_name : 'дорогая';
}
    ...
    bot.sendMessage(chatId, info[type] || info.about, {
        ...
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: '✨ Продолжить', callback_data: 'continue_to_bot' }]
            ]
        }
    });
}
...
👇 Нажми на каждую функцию, чтобы узнать подробнее:`;
...
Следи за обновлениями!`
    ...
    const backCallback = featureMap[feature] || 'features_done';
...
💗 Следи за обновлениями!`;
    ...
    if (data.startsWith('test_')) {
        ...
        if (!state || state.step !== 'test') {
            bot.sendMessage(chatId, 'Начни тест заново: нажми "🧘‍♀️ Тест состояния"');
            return;
        }
...
function startTest(chatId) {
    ...
    const name = getUsersName(String(chatId));
    bot.sendMessage(chatId,
...
// ================================================
// ===== ОБРАБОТКА ПОДПИСОК ИЗ МЕНЮ =====
// ================================================

// Добавляем обработку для кнопок подписок
bot.on('callback_query', (query) => {
    const chatId = query.message.chat.id;
    const data = query.data;
    const userId = String(chatId);
    
    // ... (остальные обработчики уже есть выше)

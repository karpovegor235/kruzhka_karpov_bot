...
const TRAINING_DATE = new Date(2026, 7, 22); // 22 августа 2026
...
function loadData() {
    try {
        if (fs.existsSync(DATA_FILE)) {
            ...
            // Добавляем новые поля, если их нет (сохраняя старые)
...
// ================================================
// ===== НОВАЯ ФУНКЦИЯ: РЕФЕРАЛЬНАЯ ССЫЛКА =====
// ================================================
...
// ================================================
// ===== НОВАЯ ФУНКЦИЯ: ОБРАБОТКА РЕФЕРАЛОВ =====
// ================================================
...
// ================================================
// ===== НОВАЯ ФУНКЦИЯ: НАПОМИНАНИЕ О ТРЕНИНГЕ =====
// ================================================

function sendTrainingReminders() {
    try {
        const today = new Date();
        const daysLeft = Math.ceil((TRAINING_DATE - today) / (1000 * 60 * 60 * 24));
        
        // Отправляем напоминания за 7, 3 и 1 день
        if (daysLeft === 7 || daysLeft === 3 || daysLeft === 1) {
            const data = loadData();
            // Собираем всех, кто интересовался тренингом или активных пользователей
            const interestedUsers = data.users.filter(u => {
                const tags = getUserTags(u.id);
                return tags.includes('training_interested') || tags.includes('active_user');
            });
            
            const reminderText = daysLeft === 1 
                ? `🔥 <b>УЖЕ ЗАВТРА, ${name}!</b>\n\nТренинг «Кружка» состоится уже завтра!\n\n📍 ${TRAINING_CITY}\n📅 22 августа\n⏰ 10:00 – 16:10\n\nЕсть последние места. Успей записаться!` 
                : `🗓️ <b>Напоминание, ${name}!</b>\n\nДо тренинга «Кружка» осталось <b>${daysLeft} дней</b>.\n\n📍 ${TRAINING_CITY}\n📅 22 августа\n⏰ 10:00 – 16:10\n\nЕщё есть места. Хочешь присоединиться?\n\n👇 Напиши @egor_provedet, чтобы забронировать место.`;
            
            for (const user of interestedUsers) {
                try {
                    const name = getUsersName(user.id);
                    bot.sendMessage(user.id,
                        reminderText.replace(/{name}/g, name),
                        {
                            parse_mode: 'HTML',
                            reply_markup: {
                                inline_keyboard: [
                    ...
                    console.log(`⚠️ Не удалось отправить напоминание пользователю ${user.id}`);
                }
            }
        }
    } catch (e) {
        console.error('❌ Ошибка в sendTrainingReminders:', e.message);
    }
}

// ================================================
// ===== НОВАЯ ФУНКЦИЯ: СБОР EMAIL =====
// ================================================
...
// ================================================
// ===== НОВАЯ ФУНКЦИЯ: ПАРСИНГ ЗАЯВКИ НА ТРЕНИНГ =====
// ================================================

function parseTrainingApplication(text) {
    // Пытаемся извлечь имя, телефон и тариф
    ...
    // Если не нашли имя, берем первую строку
...
// ================================================
// ===== ГЛАВНОЕ МЕНЮ (СУЩЕСТВУЮЩЕЕ + НОВАЯ КНОПКА) =====
// ================================================
    ...
    return {
        reply_markup: {
            keyboard: [
                ...
                [{ text: '👯‍♀️ Пригласить подругу' }] // НОВАЯ КНОПКА
            ],
...
function scheduleDailyMessages() {
    console.log('⏰ Планировщик запущен (МСК)');
    ...
    setInterval(() => {
        try {
            ...
            if (hours === 8 && minutes === 0) {
                console.log(`🌅 Отправляю утреннюю рассылку (МСК ${hours}:${minutes})...`);
                sendMorningPractice();
            }
            
            // Вечер в 22:00 МСК
            if (hours === 22 && minutes === 0) {
                console.log(`🌙 Отправляю вечернюю рассылку (МСК ${hours}:${minutes})...`);
                sendEveningQuestion();
            }
            
            // Проверяем напоминания о тренинге каждый час
            if (minutes === 0) {
...
// ================================================
// ===== ТЕСТ (СУЩЕСТВУЮЩИЙ + ДОБАВЛЕН ТРЕКИНГ) =====
// ================================================
...
function askForEmail(chatId) {
    const userId = String(chatId);
    const name = getUsersName(userId);
    
    bot.sendMessage(chatId,
        `📧 <b>Хочешь получать анонсы, ${name}?</b>

Я буду присылать на почту 1 раз в месяц самое важное:
• Анонсы тренингов и ретритов
• Полезные практики
• Вдохновляющие письма

Без спама. Только то, что действительно важно.

👇 Напиши свой email в ответ, если хочешь быть в курсе.`,
        { parse_mode: 'HTML' }
    );
...
💗 Бот — для тебя, а не ты — для бота.`;
    ...
    // ===== ТРЕНИНГ (УЛУЧШЕННЫЙ СБОР ЗАЯВОК) =====
    if (data === 'go_training') {
        bot.deleteMessage(chatId, query.message.message_id);
        userStates[chatId] = { 
...
bot.on('message', (msg) => {
    ...
    // ===== СБОР EMAIL (НОВЫЙ) =====
    if (state.step === 'email_collection') {
...
// ================================================
// ===== ОБРАБОТКА КНОПОК ГЛАВНОГО МЕНЮ (СУЩЕСТВУЮЩИЕ + НОВАЯ) =====
// ================================================
...
// ===== НОВАЯ КНОПКА: ПРИГЛАСИТЬ ПОДРУГУ =====
...
console.log('📊 Добавлены функции: рефералы, теги, аналитика, напоминания о тренинге, сбор email');

...
bot.onText(/\/start/, (msg) => {
    ...
    // ===== ПЕРВОЕ СООБЩЕНИЕ: ИСТОРИЯ ПРО КРУЖКУ =====
    const storyText = 
`☕ <b>Привет, ${firstName}!</b>

Я хочу рассказать тебе историю.

Когда мы рождаемся, у нас появляется невидимая кружка. Пустая. Чистая. Готовая наполняться.
...
• Чужие ожидания, которым пытались соответствовать
• Усталость, которой не давали выхода
• Зажимы, которые застыли в теле
...
Мы срываемся на близких. Взрываемся. Плачем без причины. Чувствуем пустоту внутри.

Но вот что я понял за годы своих практик:

<b>Кружку нельзя просто закрыть крышкой.</b>
Нельзя сделать вид, что её нет.
Нельзя сказать «возьми себя в руки» — это то же самое, что заклеить кружку скотчем.
...
Здесь можно выдохнуть. Здесь можно быть честной. Здесь можно освободиться от старого и начать наполняться новым.

Не спеши. Просто начни.`;

    bot.sendMessage(chatId, storyText, { parse_mode: 'HTML' });

    // ===== ВТОРОЕ СООБЩЕНИЕ: КТО ТАКОЙ ЕГОР =====
    setTimeout(() => {
        const aboutText = 
`━━━━━━━━━━━━━━━━━━━━━━━━━
<b>Кто я и зачем я здесь</b>

Меня зовут <b>Егор Карпов</b>.

Я — проводник. Я создаю пространство, где можно выдохнуть и услышать себя.

Более 6 лет я помогаю людям возвращать контакт с собой, снимать зажимы и находить своё состояние.

В моей работе:
🧬 Расстановки — чтобы развязать родовые узлы
🔥 Гвоздестояние — чтобы вернуть контакт с телом
🧠 Работа с подсознанием — чтобы убрать блоки

Я не даю готовых ответов. Я создаю пространство, где ты сама находишь свой путь.

<b>Что может дать тебе этот бот:</b>
🌅 <b>Утренние практики</b> — начать день с заботы о себе
📖 <b>Годовой дневник</b> — вести ежедневную рефлексию
🧘‍♀️ <b>Тест</b> — узнать, что переполняет твою кружку
🤍 <b>Пространство тишины</b> — остановиться на 30 секунд
🏆 <b>Мой путь</b> — видеть свой прогресс

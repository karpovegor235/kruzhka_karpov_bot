...
bot.onText(/📖 Годовой дневник/, (msg) => {
    const chatId = msg.chat.id;
    const userId = String(chatId);
    const stats = getJournalStats(userId);
    const today = new Date().toISOString().slice(0, 10);
    const todayEntry = getDayEntry(userId, today);
    
    let text = 
`📖 <b>Твой годовой дневник</b>

Это пространство для ежедневной рефлексии.
Каждое утро и вечер — вопрос для тебя.

━━━━━━━━━━━━━━━━━━━━━━━━━
📊 <b>Статистика:</b>
📝 Заполнено дней: ${stats.filled}
📅 Всего записей: ${stats.total}
🔥 Серия дней: ${stats.streak}

━━━━━━━━━━━━━━━━━━━━━━━━━
📅 <b>Сегодня (${today}):</b>
🌅 Утро: ${todayEntry.morning ? '✅ Записано' : '❌ Пока нет'}
🌙 Вечер: ${todayEntry.evening ? '✅ Записано' : '❌ Пока нет'}`;

    const buttons = [];
    
    if (!todayEntry.morning) {
        buttons.push([{ text: '🌅 Записать утро', callback_data: `journal_write_morning_${today}` }]);
    }
    if (!todayEntry.evening) {
        buttons.push([{ text: '🌙 Записать вечер', callback_data: `journal_write_evening_${today}` }]);
    }
    
    const dayCount = getUserDays(userId).length;
    if (dayCount > 0) {
        buttons.push([{ text: '📚 Архив записей', callback_data: `journal_archive_${userId}` }]);
    }
